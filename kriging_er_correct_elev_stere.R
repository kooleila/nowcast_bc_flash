...
library('raster')
library('outliers')
source('/home/users/hietal/statcal/R_projects/nowcasting_backup/gridding_elev_stere.R') # subroutines
source('/home/users/hietal/statcal/R_projects/nowcasting_backup/smartmet_obs_elev.R') # obs fetch from smartmet server
...
# land sea mask used
LSM <- readRDS('/home/users/hietal/statcal/R_projects/nowcasting_biascorrection/MEPS_lsm.Rds') # only sea (+Vänern and Wettern)
#coordnames(LSM) <- c('longitude','latitude') # needed for gridding (should be fixed!)
# Model elevation
MELEV <- readRDS('/home/users/hietal/statcal/R_projects/nowcasting_biascorrection/MEPS_alt.Rds')
...
if (obs_param == 'flash') {
  ...
  if (nrow(obs)>1) {
    ...
    var.pred <- gridobs(obsx,out,clen=1000*clenx)# lsm=LSM$lsm)
